...
from ui_inventory_editor import Ui_inventoryEditor
...
class mainWindow(QWidget):
    ...
    UserListWidgetCreated=False
    projectInventoryCreated=False
    ProjectCartCreated=False
    InventoryEditorCreated=False
    def __init__(self):
...
#        self.windowWidget.resize(1280, 800)
        ...
        self.userListWidget = QWidget()
        self.projectInventory = QMainWindow()
        self.projectCart = QMainWindow()
        self.inventoryEditor = QMainWindow()
        self.inventoryEditorScroller = ScrollArea()
    ...
    def createStackedPages(self):
        ...
        self.StackWidget.addWidget(self.userListWidget)
        self.StackWidget.addWidget(self.projectInventory)
        self.StackWidget.addWidget(self.projectCart)
        self.StackWidget.addWidget(self.inventoryEditorScroller)
    ...
    def setupInventoryEditor(self):
        if not self.InventoryEditorCreated:
            Ui_inventoryEditor().setupUi(self.inventoryEditor)
            self.inventoryEditorScroller.addWidget(self.inventoryEditor)
        self.categoryList = self.inventoryDb.getCatagories()
        delete = self.inventoryEditor.findChild(QPushButton, "Delete")
        ...
        save = self.inventoryEditor.findChild(QPushButton, "Save")
        plusButton = self.inventoryEditor.findChild(QPushButton, "plus")
        minusButton = self.inventoryEditor.findChild(QPushButton, "minus")
        quantity = self.inventoryEditor.findChild(QLabel, "quantity")
        new = self.inventoryEditor.findChild(QPushButton, "new_2")
        ...
        self.updateInventoryEditorItemInfo(None)
        ...
        if not self.InventoryEditorCreated:
            ...
            itemView.selectionModel().currentChanged.connect(self.updateInventoryEditorItemInfo)
            categoryView.selectionModel().currentChanged.connect(lambda:itemView.setCurrentIndex(self.itemListModel.index(0,0)))
            new.clicked.connect(lambda: self.newItem())
            save.clicked.connect(lambda: self.saveItem())
            delete.clicked.connect(lambda: self.deleteItem())
            self.InventoryEditorCreated=True
        categoryView.setCurrentIndex(self.categoryModel.index(0,0))

    def updateInventoryEditorItemInfo(self, id,previous_id=None):
        partName = self.inventoryEditor.findChild(QLineEdit, "partName")
        partCategory = self.inventoryEditor.findChild(QLineEdit, "partCategory")
        partID = self.inventoryEditor.findChild(QLabel, "partID")
        partShelf = self.inventoryEditor.findChild(QLineEdit, "partShelf")
        partBox = self.inventoryEditor.findChild(QLineEdit, "partBox")
        partImage = self.inventoryEditor.findChild(QLabel, "partImage")
        plusButton = self.inventoryEditor.findChild(QPushButton, "plus")
        minusButton = self.inventoryEditor.findChild(QPushButton, "minus")
        quantity = self.inventoryEditor.findChild(QLabel, "quantity")
        categoryView = self.inventoryEditor.findChild(QListView, "categoryView")
        rfid = self.inventoryEditor.findChild(QLineEdit, "rfid")
        
        if not self.InventoryEditorCreated:
            plusButton.clicked.connect(lambda: self.updateTotalQuantity(quantity,True))
            minusButton.clicked.connect(lambda: self.updateTotalQuantity(quantity,False))
        ...
        quantity.setText(" "+str(itemDetails[6])+" ")
    ...
    def updateTotalQuantity(self,label, increase):
        categoryView = self.inventoryEditor.findChild(QListView, "categoryView")
        itemView = self.inventoryEditor.findChild(QListView, "itemListView")
    ...
    def newItem(self):
        ...
        partName.setText("")
        partCategory.setText("")
        partID.setText(str(self.inventoryDb.maxID()+1))
        partShelf.setText("")
        partBox.setText("")
        quantity.setText(" 0 ")
        rfid.setText("")
    ...
    def saveItem(self):
        ...
        partImage = self.inventoryEditor.findChild(QLabel, "partImage")
        quantity = self.inventoryEditor.findChild(QLabel, "quantity")
        status = self.inventoryEditor.findChild(QLabel, "status")
        if self.inventoryDb.updateItem(partID.text(), partName.text(), rfid.text(), partShelf.text(), partBox.text(), partCategory.text(), quantity.text()):
            status.setText('<font color=\'green\'>'+'Inventory updated successfully'+'</font>')
        else:
            status.setText('<font color=\'red\'>Error!</font>')
        Thread(target=self.clearStatus,args=(status,)).start()
        self.updateInventoryCategories()
        categoryView.setCurrentIndex(self.categoryModel.index(0,0))
        self.updateInventoryItemList(categoryView.currentIndex())
    ...
    def deleteItem(self):
        ...
        partID = self.inventoryEditor.findChild(QLabel, "partID")
        status = self.inventoryEditor.findChild(QLabel, "status")
        ret = self.inventoryDb.deleteItem(partID.text())
        if ret==1:
            status.setText('<font color=\'green\'>'+'Item Deleted'+'</font>')
        elif ret==0:
            status.setText('<font color=\'red\'>Item in use!</font>')
        else:
            status.setText('<font color=\'red\'>Error!</font>')
        Thread(target=self.clearStatus,args=(status,)).start()
        self.updateInventoryCategories()
    ...
    def setupProjectsBrowser(self):
        ...
        leadUser = self.projectsBrowser.findChild(QPushButton, "changeLeadUser")
        inventory = self.projectsBrowser.findChild(QPushButton, "Inventory")
        cart = self.projectsBrowser.findChild(QPushButton, "ViewCart")
        self.updateUserList()
        if not self.ProjectsBrowserCreated:
            projectsView.clicked.connect(lambda:self.updateProjectInfo())
            projectsView.clicked.connect(lambda:self.updateViewProjectCart())
            ...
            leadUser.clicked.connect(lambda:self.launchWindow(12))
            inventory.clicked.connect(lambda:self.launchWindow(13))
            cart.clicked.connect(lambda:self.launchWindow(14))
            self.ProjectsBrowserCreated = True
            
    def setupUserListWidget(self):
        if not self.UserListWidgetCreated:
            Ui_userList().setupUi(self.userListWidget)
        usersView = self.userListWidget.findChild(QListView, "userView")
        selectButton = self.userListWidget.findChild(QPushButton, "Select")
        self.updateUserList()
        usersView.setModel(self.userModel)
        if not self.UserListWidgetCreated:
            selectButton.clicked.connect(lambda:self.updateLead())
            self.UserListWidgetCreated=True
        
    def updateLead(self):
        usersView = self.userListWidget.findChild(QListView, "userView")
        if usersView.currentIndex().row():
    ...
    def setupProjectInventory(self):
        if not self.projectInventoryCreated:
            Ui_inventoryWindow().setupUi(self.projectInventory)
        ...
        #self.categoryList = self.inventoryDb.getCatagories()
        cartButton = self.projectInventory.findChild(QPushButton, "cartButton")
        categoryView = self.projectInventory.findChild(QListView, "categoryView")
        itemView = self.projectInventory.findChild(QListView, "itemListView")
        addToCartButton = self.projectInventory.findChild(QPushButton, "addToCartButton")
        plusButton = self.projectInventory.findChild(QPushButton, "plus")
        minusButton = self.projectInventory.findChild(QPushButton, "minus")
        quantity = self.projectInventory.findChild(QLabel, "quantity")
        partQty = self.projectInventory.findChild(QLabel, "partQty")
        ...
        self.updateProjectInventoryItemInfo(None)
    ...
    def updateProjectInventoryItemInfo(self, id,previous_id=None):
        partName = self.projectInventory.findChild(QLabel, "partName")
        partCategory = self.projectInventory.findChild(QLabel, "partCategory")
        partID = self.projectInventory.findChild(QLabel, "partID")
        partShelf = self.projectInventory.findChild(QLabel, "partShelf")
        partBox = self.projectInventory.findChild(QLabel, "partBox")
        partQty = self.projectInventory.findChild(QLabel, "partQty")
        partImage = self.projectInventory.findChild(QLabel, "partImage")
        plusButton = self.projectInventory.findChild(QPushButton, "plus")
        minusButton = self.projectInventory.findChild(QPushButton, "minus")
        quantity = self.projectInventory.findChild(QLabel, "quantity")
        categoryView = self.projectInventory.findChild(QListView, "categoryView")
        ...
        if not self.projectInventoryCreated:
            plusButton.clicked.connect(lambda: self.updateQuantityProject(quantity,True))
            minusButton.clicked.connect(lambda: self.updateQuantityProject(quantity,False))
    ...
    def updateQuantityProject(self,label, increase):
        categoryView = self.projectInventory.findChild(QListView, "categoryView")
        itemView = self.projectInventory.findChild(QListView, "itemListView")
    ...
    def addToProjectCartAction(self, itemView, qtySpinBox, partQty):
        status=self.projectInventory.findChild(QLabel, "status")
        if len(itemView.selectedIndexes()) != 0:
            ...
            if self.inventoryDb.addToProjectCart(self.current_project, self.user.getName(), itemId, int(qtySpinBox.text()), ""):
                status.setText('<font color=\'green\'>'+qtySpinBox.text()+ ' '+itemName+' added to cart'+'</font>')
            else:
                status.setText('<font color=\'red\'>Error!</font>')
            Thread(target=self.clearStatus,args=(status,)).start()
            self.updateProjectInventoryItemInfo(itemView.currentIndex())
    ...
    def setupProjectCart(self):
        if not self.ProjectCartCreated:
            Ui_cartWindow().setupUi(self.projectCart)
        self.project_cart_model = QStandardItemModel()
        listView = self.projectCart.findChild(QListView, "listView")
        #buttonBox = self.cart.findChild(QDialogButtonBox, "buttonBox")
        #openInventory = self.cart.findChild(QPushButton, "openInventory")
        removeCartButton = self.projectCart.findChild(QPushButton, "removeCartButton")
        partID = self.projectCart.findChild(QLabel, "partID")
        quantity = self.projectCart.findChild(QLabel, "quantity")
        plusButton = self.projectCart.findChild(QPushButton, "plus")
        minusButton = self.projectCart.findChild(QPushButton, "minus")
        ...
        if not self.ProjectCartCreated:
            removeCartButton.clicked.connect(lambda: self.removeFromProjectCartAction(listView, int(str(partID.text()))))
            listView.clicked.connect(self.displayProjectCartItem)
            plusButton.clicked.connect(lambda: self.updateQuantityProjectCart(quantity,True))
            minusButton.clicked.connect(lambda: self.updateQuantityProjectCart(quantity,False))
            saveButton.clicked.connect(lambda: self.modifyProjectCartQuantity(quantity))
            #openInventory.clicked.connect(lambda: self.launchWindow(5))
            self.ProjectCartCreated=True
            
    def updateViewProjectCart(self):
        self.project_cart_model.clear()
        itemList = self.projectDb.getItemList(self.current_project)
        for item in itemList:
            self.project_cart_model.appendRow(QStandardItem(item))

    def displayProjectCartItem(self, itemId,previous_id=None):
        listView = self.projectCart.findChild(QListView, "listView")
        partName = self.projectCart.findChild(QLabel, "partName")
        partCategory = self.projectCart.findChild(QLabel, "partCategory")
        partID = self.projectCart.findChild(QLabel, "partID")
        partShelf = self.projectCart.findChild(QLabel, "partShelf")
        partBox = self.projectCart.findChild(QLabel, "partBox")
        partQty = self.projectCart.findChild(QLabel, "quantity")
        issueStatus = self.projectCart.findChild(QLabel, "issueStatus")
        partImage = self.projectCart.findChild(QLabel, "partImage")
        removeCartButton = self.projectCart.findChild(QPushButton, "removeCartButton")
        plusButton = self.projectCart.findChild(QPushButton, "plus")
        minusButton = self.projectCart.findChild(QPushButton, "minus")
        ...
        self.projectDb.getItemList(self.current_project)
        itemName = '\'' + itemId.data() + '\''
        itemId = self.projectDb.item_list[listView.currentIndex().row()][3]
        if self.projectDb.item_list[listView.currentIndex().row()][6]=='':
    ...
    def updateQuantityProjectCart(self,label, increase):
        saveButton = self.projectCart.findChild(QPushButton, "cartButton")
        listView = self.projectCart.findChild(QListView, "listView")
        ...
        maxquantity=int(self.projectDb.item_list[listView.currentIndex().row()][4])
        if increase:
            label.setText(" "+str(min((int(label.text())+1),maxquantity))+" ")
            label.update()
        else:
    ...
    def modifyProjectCartQuantity(self,label):
        status=self.projectCart.findChild(QLabel, "status")
        partName = self.projectCart.findChild(QLabel, "partName")
        listView = self.projectCart.findChild(QListView, "listView")
        if listView.currentIndex().row()==None:
            print(listView.currentIndex().row())
            return
        self.projectDb.getItemList(self.current_project)
        itemId=int(self.projectDb.item_list[listView.currentIndex().row()][3])
        quantity=int(label.text())  
        issued_quantity= int(self.projectDb.item_list[listView.currentIndex().row()][4]) 
        returned_quantity=issued_quantity-quantity
        if self.projectDb.changeQuantity(self.current_project,itemId,quantity):
            status.setText('<font color=\'green\'>'+str(returned_quantity)+' '+partName.text()+' returned</font>')
            Thread(target=self.clearStatus,args=(status,)).start()
    
    def removeFromProjectCartAction(self, listView, partID):
        status=self.projectCart.findChild(QLabel, "status")
        if len(listView.selectedIndexes()) != 0:
            self.projectDb.removeFromCart(self.current_project, partID)
            self.updateViewProjectCart()
    ...
    def setupAdmin(self):
        ...
        inventoryEditorButton = self.admin.findChild(QPushButton, "pushButton")
        ...
        if not self.AdminCreated:
            ...
            inventoryEditorButton.clicked.connect(lambda: self.launchWindow(15))
    ...
    def updateInventoryCategories(self):
        self.categoryModel.clear()
        self.categoryList = self.inventoryDb.getCatagories()
        for item in self.categoryList:
            self.categoryModel.appendRow(QStandardItem(item))
...
#        self.setupFingerprint()
        ...
        self.setupUserListWidget()
        self.setupProjectInventory()
        self.setupProjectCart()
        self.setupInventoryEditor()
