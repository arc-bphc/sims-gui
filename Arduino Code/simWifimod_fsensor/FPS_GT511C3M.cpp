...
FPS_GT511C3::FPS_GT511C3(bool dubug)
{
  _serial.begin(9600);
...
int FPS_GT511C3::SetTemplate(int id, byte temp[],bool duplicateCheck)
{
  ...
  delete rp;
  if(retval){
    SendCommand(temp, 504);
    Serial.println("sent template"+String(_serial.available()));
    Response_Packet* rp = GetResponse();
